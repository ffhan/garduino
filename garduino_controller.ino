...
class Control{
   ...
   * |  2  |   0   | WRITTEN | 1 - CURRENT WRITE CYCLE COMPLETED, 0 - NOT COMPLETED
  ...
  private : typedef enum {LOCK = 0, LOGGING = 1, WRITTEN = 2, IS_INITIALISED = 3, 
                        ...
                        FAN_SPEED = 10, FAN_ADMIN = 13} sector;
...
void loop() {
    ...
    if(!sys.getLightAdmin()){
        ...
        if(getDecimalTime(now) >= 8.0 && getDecimalTime(now) < 22.0){ //TODO: not constantly turning on
          ...
          if(!sys.getLightingState()) sys.setLightingState(1);
        } else{
          digitalWrite(lightControlPin, HIGH); // turn the light off.
          if(sys.getLightingState()) sys.setLightingState(0);
        }
     }
