...
void buildUpdateStateRoute(char *route) {
  int index = 0;
  index += dumpFragment(updateFrnt, route);
  index += dumpEeprom(460, route + index);
  index += dumpFragment(updateBack, route + index);
  route[index - 2] = '\r';
  route[index - 1] = '\n';
  route[index] = '\0';
}
...
void writeUpdateContent() {
  writeFragment(contLen);
  int len = buildUpdateJson(&fragmentLen, &valueLen, &eepromLen);
  client.print(len);
  Serial.print(len);
  writeNewLine();
  writeNewLine();
  buildUpdateJson(&processFragment, &processValue, &processEeprom);
}
...
void updateState() {
  char route[80] = "test";
  buildUpdateStateRoute(route);
  poster(route, true, writeUpdateContent, true);
  printResponse();
}
