...
Screen::Screen(byte cols, byte rows, Menu *mainMenu) {
	...
	this->cols = cols;
	bindToMenu(mainMenu);
	lcd = new LiquidCrystal(12, 11, 5, 13, 10, 2);
	lcd->begin(cols, rows);
}
...
void Screen::valueSettingTrailPrint(Item *item) {
	...
	if (item->getType() == BOOLSETTING || item->getType() == BOOLBITSETTING) {
		if (setting->getTempValue()) {
			Serial.print(trueSign);
			lcd->print(trueSign);
		}
		else { 
			Serial.print(falseSign); 
			lcd->print(falseSign); 
		}
	}
	else {
		Serial.print(setting->getTempValue());
		lcd->print(setting->getTempValue());
	}
