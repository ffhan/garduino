...
int WebController::processValue(char *value) {
  client.print(value);
  Serial.print(value);
  return strlen(value);
}
int WebController::valueLen(char *value) {
  return strlen(value);
}

void WebController::writeFragment(char *fragment) { // fragment NEEDS string escape char.
...
int WebController::dumpFragment(char *fragment, char *dest) {
  char buff = pgm_read_byte_near(fragment);
  int i = 0;
  while (buff != '\0') {
    *(dest + i) = buff;
...
int WebController::processFragment(char *fragment) {
  char buff = pgm_read_byte_near(fragment);
  int i = 0;
  while (buff != '\0') {
    i++;
...
int WebController::fragmentLen(char *fragment) {
  char buff = pgm_read_byte_near(fragment);
  int i = 0;
  while (buff != '\0') {
    i++;
...
void WebController::writeRoute(char *route) {
...
void WebController::writeStaticRoute(char *route) {
  client.print(route);
  Serial.print(route);
}

void WebController::getter(char *route, bool staticPath) {
  if (connectToServ() > 0) {
    this->printMakeHttp();
...
//USE FOR POST & PUT
void WebController::poster(char *route, bool writeToken, ContentWriter cw, bool staticPath) {
  if (connectToServ() > 0) {
    this->printMakeHttp();
...
void WebController::parseResponse(char *before, char *after, int *action, bool eepromWrite, bool writeNewLine, int eepromIndex) {
  char actStrg[30] = "";
